...
XMLTreeLoader.prototype.notify = function() {
	for( var i=0; i<this.subscribers.length; i++ ) {
		this.subscribers[i].notify(this.rootNode);
	}
}

XMLTreeLoader.prototype.handleRequest = function( request ){
	this.handle(request.responseXML);
}
/*
 ...
 */
XMLTreeLoader.prototype.handle = function( xmlDoc ) {
...
XMLTreeLoader.prototype.branch = function( root, rootNode ) {
	...
	for( var i=0, l=childNodes.length; i<l; i++ ){
		if( childNodes[i].nodeName == "node" ) {
			...
			if ( !this.nodesByName[node.text] ) {
				this.nodesByName[node.text] = node;
				this.dataGraph.addNode( node );
			} else {
				var otherNode = this.nodesByName[node.text];
				this.dataGraph.addEdge( node.parent, otherNode );
			}
